...
#Mehr über Simulationen und Verteilungen am 8.11
...
# Weitere (elegantere) Variante: 
# Erstellen einer Funktion die für jede Spalte die Elemente zurückgibt,
# die die Bedingungen erfüllen. Anwendung mit apply auf den Datensatz

rm.elements<-function(data, lower, upper){
if(is.numeric(data)){
res<-data[which(((data > lower) &  (data < upper)) & (!is.na(data))  )]
}
else{res=NA}
length(res)
}
...
rm.elements<-function(data, lower, upper){
res<-data[which(((data > lower) &  (data < upper)) & (!is.na(data)) & (is.numeric(data)) )]
length(res)
}
...
rm.elements(test, lower=2, upper=4)

apply(data, MARGIN=2, rm.elements, lower=20, upper=30)

sum(apply(data, MARGIN=2, rm.elements, lower=20, upper=30))
