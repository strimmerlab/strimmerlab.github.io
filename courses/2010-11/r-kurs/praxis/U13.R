#Loesung des dreizehnten Uebungsblattes
...
data.names = golub.gnames[ranking.t[1:250],1]
data = (golub[ranking.t[1:250],])

row.names(data) = data.names
data = t(data)

#(c) 
golub.pcor <- ggm.estimate.pcor(data, method = "static")
golub.edges <- ggm.test.edges(golub.pcor, direct=TRUE)

#(d)
golub.net <- extract.network(golub.edges, method.ggm="number", cutoff.ggm=100,
method.dir="prob", verbose = TRUE)
